...
/* ------------------------------------------------------------------ */
/*  Floaty: gentle idle bob (no cursor tracking — cheaper + calmer)   */
/* ------------------------------------------------------------------ */
...
function Floaty({
  ...
  return (
    <motion.div style={{ zIndex: z }} className={`absolute ${className ?? ""}`}>
      <motion.div
        style={{ rotate }}
        animate={{ y: [0, -float, 0] }}
...
function SubjectCard({ title, tag, variant }: { title: string; tag: string; variant: keyof typeof cardVariants }) {
  return (
    <div
      className={`w-[252px] h-[150px] rounded-[1.6rem] p-6 shadow-[0_30px_60px_-22px_rgba(0,0,0,0.45)] border border-white/40 flex flex-col justify-between ${cardVariants[variant]}`}
    >
...
const FAN = [
  { title: "Math", tag: "Thu · 19:00", variant: "amber", pos: "left-[-2%] top-[13%]", rotate: -16, float: 9, dur: 7.5 },
  { title: "English", tag: "Tue · 22:00", variant: "light", pos: "left-[4%] top-[22%]", rotate: -9, float: 8, dur: 8 },
  { title: "Networking", tag: "Mon · 20:00", variant: "dark", pos: "left-[10%] top-[31%]", rotate: -3, float: 11, dur: 6.5 },
  { title: "Database", tag: "Wed · 21:30", variant: "green", pos: "left-[14%] top-[38%]", rotate: 5, float: 9, dur: 7 },
  { title: "JavaScript", tag: "Mon · 20:30", variant: "blue", pos: "left-[19%] top-[46%]", rotate: 12, float: 12, dur: 8.5 },
] as const;
      ...
      <div className="relative overflow-hidden bg-[#ececec] dark:bg-[#0a0e17] min-h-screen flex flex-col">
        ...
        <div className="relative flex-1 flex flex-col items-center justify-center text-center px-6 pb-24 pt-24">
          ...
          <div className="hidden lg:block">
            <div className="absolute left-[3%] top-[28%] font-display font-bold text-[10rem] leading-none text-black/[0.04] select-none pointer-events-none -rotate-12">
              Mon
            </div>

            {FAN.map((c, i) => (
              <Floaty key={c.title} rotate={c.rotate} className={c.pos} z={20 + i} float={c.float} duration={c.dur} delay={i * 0.2}>
                <SubjectCard title={c.title} tag={c.tag} variant={c.variant} />
              </Floaty>
            ))}

            {/* cursor */}
            <Floaty rotate={-10} className="left-[17%] top-[56%]" z={26} float={7} duration={6}>
              <MousePointer2 size={54} className="text-white fill-white drop-shadow-[0_8px_14px_rgba(0,0,0,0.35)]" />
            </Floaty>
            ...
            {/* lightning */}
            <Floaty rotate={-8} className="right-[14%] top-[54%]" z={32} float={14} duration={6.8} delay={0.4}>
              <div className="w-[98px] h-[98px] rounded-[1.8rem] bg-slate-900 border-4 border-white shadow-[0_28px_50px_-16px_rgba(0,0,0,0.5)] flex items-center justify-center">
                <Zap size={46} className="text-amber-400 fill-amber-400" />
              </div>
            </Floaty>

            {/* fingerprint */}
            <Floaty rotate={7} className="right-[26%] top-[64%]" z={31} float={12} duration={7.2} delay={0.6}>
              <div className="w-[88px] h-[88px] rounded-[1.8rem] bg-white/70 backdrop-blur-md border-4 border-white shadow-[0_28px_50px_-16px_rgba(0,0,0,0.4)] flex items-center justify-center">
                <Fingerprint size={44} className="text-blue-500" />
              </div>
            </Floaty>

            {/* padlock */}
            <Floaty rotate={11} className="right-[7%] top-[65%]" z={34} float={15} duration={7} delay={0.2}>
              <div className="w-[100px] h-[100px] rounded-[2rem] bg-gradient-to-br from-blue-500 to-blue-600 border-4 border-white shadow-[0_30px_56px_-16px_rgba(37,99,235,0.6)] flex items-center justify-center">
