...
import { Zap, Check, Calendar as CalendarIcon, Clock, Flame, Trophy, Target } from "lucide-react";

interface PhoneMockupProps {
  screen?: "dashboard" | "calendar" | "tasks";
}

export default function PhoneMockup({ screen = "dashboard" }: PhoneMockupProps) {
  return (
    <motion.div 
        ...
        {screen === "dashboard" && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom duration-700">
            ...
            <div className="flex justify-between items-center">
               <div>
                  <h4 className="text-xl font-black text-slate-900 leading-tight">Hello, <br />Scholar</h4>
               </div>
            ...
            <div className="space-y-3">
              ...
              {[
                { t: "Networking", s: "20:00", type: "HW", done: true },
                { t: "Javascript", s: "20:30", type: "HW", done: true },
                { t: "Embedded", s: "20:00", type: "HW", done: false }
              ].map((task, i) => (
                <div key={i} className={`p-4 rounded-2xl border flex items-center gap-4 ${task.done ? 'bg-slate-50 border-slate-100 opacity-60' : 'bg-white border-slate-100 shadow-sm'}`}>
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${task.done ? 'bg-blue-600 border-blue-600 text-white' : 'border-slate-200'}`}>
        ...
        {screen === "calendar" && (
          <div className="space-y-6 animate-in fade-in slide-in-from-right duration-700">
             ...
             <div className="grid grid-cols-7 gap-2">
              {[...Array(31)].map((_, i) => (
                <div key={i} className={`aspect-square rounded-xl flex items-center justify-center text-[10px] font-black ${i === 14 ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30' : 'text-slate-400 bg-slate-50'}`}>
                  {i + 1}
                </div>
              ))}
            </div>
                ...
                {[
                  ...
                  <div key={i} className="glass-container p-5 rounded-3xl border-slate-100 space-y-3">
                     <div className="flex justify-between items-start">
                        <span className={`text-[8px] font-black px-2 py-1 rounded-full uppercase tracking-widest ${item.c === 'blue' ? 'bg-blue-100 text-blue-600' : 'bg-orange-100 text-orange-600'}`}>
                           {item.t}
                        </span>
