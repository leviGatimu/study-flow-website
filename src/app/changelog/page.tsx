            ...
            <ScrollReveal key={e.version} delay={i * 0.05}>
              <div className="grid md:grid-cols-[160px_1fr] gap-6 md:gap-10 pb-10">
                ...
                <div className="md:text-right md:pt-6">
                  <div className="inline-flex md:flex-col items-center md:items-end gap-2">
                    <span
                      className={`font-display text-2xl font-semibold tracking-tight ${
                        e.upcoming ? "text-slate-400" : "text-slate-900"
                      }`}
                    >
                      {e.version}
                    </span>
                  ...
                  <p className="text-sm text-slate-400 mt-1">{e.date}</p>
                </div>

                {/* card */}
                <div
                  className={`relative bg-white rounded-[1.75rem] p-8 md:p-9 shadow-[0_28px_60px_-40px_rgba(15,23,42,0.3)] ${
                    e.upcoming ? "border-2 border-dashed border-black/10" : "border border-black/[0.06]"
                  }`}
                >
                  <div className="space-y-7">
                    {e.groups.map((g) => (
                      <div key={g.heading}>
                        <h3 className="font-display text-lg font-semibold text-slate-900 mb-3">{g.heading}</h3>
                        <ul className="space-y-2.5">
                          {g.items.map((it) => (
                            <li key={it.text} className="flex items-start gap-3">
                              ...
                              <span className="text-slate-600 leading-relaxed">{it.text}</span>
                            </li>
                          ))}
                        </ul>
                      </div>
                  ...
                  {e.footerLink && (
                    <Link
                      ...
                      className="inline-flex items-center gap-2 mt-7 text-blue-600 font-semibold hover:gap-3 transition-all"
                    >
